...
#if BIND_AprPollcbT_MALLOC
mrb_value
mrb_APR_AprPollcbT_malloc(mrb_state* mrb, mrb_value self) {
  apr_pollcb_t* native_object = (apr_pollcb_t*)malloc(sizeof(apr_pollcb_t));
  return mruby_box_apr_pollcb_t(mrb, native_object);
}
#endif
...
#if BIND_AprPollcbT_MALLOC
  mrb_define_class_method(mrb, AprPollcbT_class, "malloc", mrb_APR_AprPollcbT_malloc, MRB_ARGS_NONE());
#endif
