...
#if BIND_AprSocketT_MALLOC
mrb_value
mrb_APR_AprSocketT_malloc(mrb_state* mrb, mrb_value self) {
  apr_socket_t* native_object = (apr_socket_t*)malloc(sizeof(apr_socket_t));
  return mruby_box_apr_socket_t(mrb, native_object);
}
#endif
...
#if BIND_AprSocketT_MALLOC
  mrb_define_class_method(mrb, AprSocketT_class, "malloc", mrb_APR_AprSocketT_malloc, MRB_ARGS_NONE());
#endif
