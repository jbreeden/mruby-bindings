...
mrb_value
mrb_APR_AprHashIndexT_free(mrb_state* mrb, mrb_value self) {
  mrb_value to_free;
  mrb_get_args(mrb, "o", &to_free);

  if (!mrb_obj_is_kind_of(mrb, to_free, mrb_class_ptr(self))) {
    mrb_raise(mrb, E_TYPE_ERROR, "APR::AprHashIndexT.free can only free objects of type APR::AprHashIndexT");
    return mrb_nil_value();
  }

  apr_hash_index_t * native_to_free = mruby_unbox_apr_hash_index_t(to_free);
...
mrb_value
mrb_APR_AprHashIndexT_clear_pointer(mrb_state* mrb, mrb_value self) {
  mrb_value ruby_object;
  mrb_get_args(mrb, "o", &ruby_object);

  if (!mrb_obj_is_kind_of(mrb, ruby_object, mrb_class_ptr(self))) {
    mrb_raise(mrb, E_TYPE_ERROR, "APR::AprHashIndexT.clear_pointer can only clear objects of type APR::AprHashIndexT");
    return mrb_nil_value();
  }
...
mrb_value
mrb_APR_AprHashIndexT_address_of(mrb_state* mrb, mrb_value self) {
  mrb_value ruby_object;
  mrb_get_args(mrb, "o", &ruby_object);

  if (!mrb_obj_is_kind_of(mrb, ruby_object, mrb_class_ptr(self))) {
    mrb_raise(mrb, E_TYPE_ERROR, "APR::AprHashIndexT.address_of can only get the address for objects of type APR::AprHashIndexT");
    return mrb_nil_value();
  }

  apr_hash_index_t * native_object = mruby_unbox_apr_hash_index_t(ruby_object);
...
void mrb_APR_AprHashIndexT_init(mrb_state* mrb) {
  ...
  mrb_define_class_method(mrb, AprHashIndexT_class, "address_of", mrb_APR_AprHashIndexT_address_of, MRB_ARGS_ARG(1, 0));
